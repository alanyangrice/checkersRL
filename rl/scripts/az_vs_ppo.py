...
# Specific epochs, more games, more MCTS sims:
    python -m rl.scripts.az_vs_ppo --az-epoch 100 --ppo-epoch 200 --games 20 --simulations 200

# AZ vs a league agent:
    python -m rl.scripts.az_vs_ppo --az-epoch 100 --agent aggressive --games 20

# AZ vs AZ  (v2 epoch 160 vs v1 epoch 135, 20 games, 200 sims each):
    python -m rl.scripts.az_vs_ppo --az2-epoch 135 --az2-version v1 --az-epoch 160 --games 20 --simulations 200

# AZ vs AZ with independent sim budgets:
    python -m rl.scripts.az_vs_ppo --az2-epoch 135 --az2-version v1 --az2-simulations 100 --simulations 200
"""
...
from rl.eval.mcts_evaluate import play_eval_game, make_mcts_agent
...
def load_az(device, epoch=None, version="v2"):
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    dir_name = "alphazero_checkpoints_v1" if version == "v1" else "alphazero_checkpoints"
    az_dir = os.path.join(base, "training_results", "mcts", dir_name)

    if epoch is not None:
        path = os.path.join(az_dir, f"az_epoch_{epoch}.pt")
    else:
        path = find_latest_checkpoint_path(az_dir, prefix="az_epoch_")
        if path is None:
...
def load_ppo(device, epoch=None, agent_type=None):
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    if agent_type is not None:
        model_dir = os.path.join(base, "training_results", "ppo", f"ppo_saved_models_{agent_type}")
        prefix = "agent_epoch_"
        label_prefix = f"PPO-{agent_type}"
    else:
        model_dir = os.path.join(base, "training_results", "ppo", "ppo_saved_models_parallel")
        prefix = "agent_epoch_"
    ...
    if epoch is not None:
        ...
        path = find_latest_checkpoint_path(model_dir, prefix="agent_epoch_")
        if path is None:
    ...
    load_policy_state_dict(net, ckpt["model_state_dict"])
    net.eval()
...
def make_ppo_agent(network, device):
...
def print_match_header(label_a, label_b, num_games, sims_a, sims_b=None):
...
def run_match_loop(agent_a, label_a, agent_b, label_b, num_games,
                   max_moves, stochastic_opening, sims_a, sims_b=None):
    """Generic match loop: agent_a vs agent_b, A plays BLUE first half."""
    half = num_games // 2
    a_wins = a_losses = ties = 0
    total_moves = 0

    col_w = print_match_header(label_a, label_b, num_games, sims_a, sims_b)

    for i in range(num_games):
        a_color = BLUE if i < half else RED

        if a_color == BLUE:
            winner, moves = play_eval_game(agent_a, agent_b,
                                            max_moves=max_moves,
                                            stochastic_opening=stochastic_opening)
        else:
            winner, moves = play_eval_game(agent_b, agent_a,
                                            max_moves=max_moves,
                                            stochastic_opening=stochastic_opening)
...
def run_match(az_net, az_label, ppo_net, ppo_label, device,
    ...
    az_agent = make_mcts_agent(az_net, device, num_simulations,
                                stochastic_opening_moves=stochastic_opening)
    ppo_agent = make_ppo_agent(ppo_net, device)

    run_match_loop(
        az_agent, az_label, ppo_agent, ppo_label, num_games,
        max_moves=cfg.MAX_GAME_MOVES_FULL,
        stochastic_opening=stochastic_opening,
        sims_a=num_simulations,
    )
...
def run_az_vs_az_match(net_a, label_a, net_b, label_b, device,
    ...
    agent_a = make_mcts_agent(net_a, device, sims_a,
                               stochastic_opening_moves=stochastic_opening)
    agent_b = make_mcts_agent(net_b, device, sims_b,
                               stochastic_opening_moves=stochastic_opening)

    run_match_loop(
        agent_a, label_a, agent_b, label_b, num_games,
        max_moves=cfg.MAX_GAME_MOVES_FULL,
        stochastic_opening=stochastic_opening,
...
if __name__ == "__main__":
    ...
    az_net, az_label = load_az(device, epoch=args.az_epoch, version=args.az_version)

    if args.az2_epoch is not None:
        # ── AZ vs AZ mode ────────────────────────────────────────────────────
        az2_net, az2_label = load_az(device, epoch=args.az2_epoch, version=args.az2_version)
        ...
        # ── AZ vs PPO mode ───────────────────────────────────────────────────
        ppo_net, ppo_label = load_ppo(device, epoch=args.ppo_epoch, agent_type=args.agent)
